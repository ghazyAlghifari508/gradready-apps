...
export default function SavedJobsPage() {
  ...
  const fetchJobs = async (signal?: AbortSignal) => {
    try {
      const res = await fetch("/api/jobs/saved", { signal });
    ...
    <div className="max-w-[1200px] mx-auto w-full" style={{ padding: "32px" }}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
        <div>
          ...
          <p className="text-[#777777] font-semibold">
            Pantau progres lamaran kerjamu di satu tempat.
          </p>
        </div>
      ...
      {isAdding && (
        <Card className="p-6 mb-8 border-t-4 border-[#1CB0F6]">
          <h3 className="text-[#4B4B4B] text-[16px] font-bold mb-4 uppercase">
            Data Lamaran Baru
          </h3>
          <form
          ...
          >
            ...
            <Button
              ...
              variant={companyName && position ? "primary" : "ghost"}
            >
              SIMPAN
            </Button>
          </form>
      ...
      {loading ? (
        <div className="text-center p-8 text-[#AFAFAF] font-bold animate-pulse">
          Memuat data tracker...
        </div>
      ) : jobs.length === 0 ? (
        <div className="text-center py-20 px-6 border-2 border-dashed border-[#E5E5E5] rounded-3xl">
          ...
          <h3 className="font-bold text-[20px] text-[#4B4B4B] mb-2">
            Belum ada lamaran tersimpan
          </h3>
          <p className="text-[#AFAFAF] font-medium mb-6">
            Mulai catat proses interview dan lamaranmu agar lebih terstruktur!
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {jobs.map((job) => (
            <Card
              key={job.id}
              className="p-5 flex flex-col justify-between border-t-8"
              style={{ borderTopColor: STATUS_COLORS[job.status] || "#E5E5E5" }}
            >
              <div className="mb-4">
                <div className="flex justify-between items-start mb-2">
                  <div
                    className="text-[12px] font-bold px-2 py-1 rounded-[8px] text-white tracking-[0.5px]"
                    style={{
                      backgroundColor: STATUS_COLORS[job.status] || "#E5E5E5",
                    }}
                  >
                    {job.status}
                  </div>
                  {confirmDeleteId === job.id ? (
                    <div className="flex items-center gap-1">
                      <span className="text-[11px] font-bold text-[#FF4B4B]">Yakin?</span>
                      <button
                        onClick={() => handleDelete(job.id)}
                        className="text-[11px] font-bold text-white bg-[#FF4B4B] rounded px-2 py-0.5"
                      >
                        Ya
                      </button>
                      <button
                        onClick={() => setConfirmDeleteId(null)}
                        className="text-[11px] font-bold text-[#777777]"
                      >
                        Batal
                      </button>
                    </div>
                ...
                <h3 className="font-bold text-[20px] text-[#4B4B4B]">
                  {job.position}
                </h3>
                <p className="font-semibold text-[#777777] text-[15px]">
                  {job.companyName}
                </p>
                <div className="text-[12px] text-[#AFAFAF] font-medium mt-1">
                  Disimpan: {new Date(job.savedAt).toLocaleDateString("id-ID")}
                </div>
              </div>

              <div className="pt-4 border-t-2 border-[#F5F5F5]">
                <p className="text-[12px] font-bold text-[#AFAFAF] uppercase tracking-[1px] mb-2">
                  Ubah Status
                </p>
                <select
                  value={job.status}
